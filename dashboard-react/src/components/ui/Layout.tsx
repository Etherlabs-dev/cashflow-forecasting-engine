      ...
      >
        ...
        <div className="p-4 border-t border-slate-800">
          <div className={`bg-slate-800/50 rounded-lg transition-all duration-300 ${isCollapsed ? 'p-2 flex justify-center' : 'p-3'}`}>
             {isCollapsed ? (
                <div className={`w-2 h-2 rounded-full ${isSupabaseConfigured ? 'bg-emerald-500' : 'bg-amber-500'} my-1`} title={isSupabaseConfigured ? 'Data source configured' : 'Synthetic demo'}></div>
             ) : (
                <div className="text-xs text-slate-400 whitespace-nowrap overflow-hidden">
                    ...
                    {isSupabaseConfigured ? 'Data source configured' : 'Synthetic demo'}
                    </div>
                    {isSupabaseConfigured ? 'Check source freshness in run metadata' : 'No live financial data loaded'}
                </div>
             )}
          </div>
        </div>
