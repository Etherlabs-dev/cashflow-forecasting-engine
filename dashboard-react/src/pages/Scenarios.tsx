...
export const Scenarios = () => {
  ...
  const handleCreateScenario = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const newSc = await dataService.createScenario(newScenarioName, growthAdj, payrollAdj);
  ...
  const baselineMinimum = baselineData.length
    ? Math.min(...baselineData.map(item => item.base_closing_balance))
    : 0;
  const scenarioMinimum = scenarioData.length
    ? Math.min(...scenarioData.map(item => item.base_closing_balance))
    : 0;
  const scenarioRunway = scenarioData.findIndex(item => item.base_closing_balance <= 0);
  const baselineRunway = baselineData.findIndex(item => item.base_closing_balance <= 0);
  const runwayDelta =
    scenarioRunway === -1 || baselineRunway === -1 ? null : scenarioRunway - baselineRunway;
            ...
            {activeScenario !== 'baseline' && (
                <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-800">
                    <div className="text-center">
                        ...
                        <div className="text-xl font-bold text-rose-400">{formatCurrency(scenarioMinimum - baselineMinimum)}</div>
                    </div>
                     <div className="text-center">
                        <div className="text-xs text-slate-500 mb-1">Runway Impact</div>
                        <div className="text-xl font-bold text-amber-400">{runwayDelta === null ? 'Not crossed' : `${runwayDelta} Days`}</div>
                    </div>
                     <div className="text-center">
                        <div className="text-xs text-slate-500 mb-1">Evidence</div>
                        <div className="text-xl font-bold text-slate-200">Synthetic</div>
                    </div>
                </div>
            )}
          </Card>
