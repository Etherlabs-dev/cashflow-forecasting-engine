...
export const Dashboard = () => {
  ...
  const asOfDate = forecasts[0]?.forecast_as_of_date || actuals[actuals.length - 1]?.date;
  
  // Find runway
  const zeroDateBase = forecasts.find(f => f.base_closing_balance <= 0)?.date;
  const daysRunwayBase = zeroDateBase && asOfDate
    ? Math.floor((new Date(`${zeroDateBase}T00:00:00Z`).getTime() - new Date(`${asOfDate}T00:00:00Z`).getTime()) / (1000 * 3600 * 24))
    : '90+';

  const zeroDateWorst = forecasts.find(f => f.worst_closing_balance <= 0)?.date;
  const daysRunwayWorst = zeroDateWorst && asOfDate
    ? Math.floor((new Date(`${zeroDateWorst}T00:00:00Z`).getTime() - new Date(`${asOfDate}T00:00:00Z`).getTime()) / (1000 * 3600 * 24))
    : '90+';
  ...
  const formatCurrency = (val: number) => {
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: forecasts[0]?.currency || 'USD', maximumFractionDigits: 0 }).format(val);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-400 max-w-xl">
            Frozen, assumption-driven projections with explicit synthetic and production evidence labels.
          </p>
        </div>
        <div className="hidden md:block">
           ...
           <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-900/50 px-3 py-1.5 rounded-full border border-slate-800">
             ...
             {forecasts[0]?.evidence_label === 'synthetic' ? 'Synthetic demonstration' : 'Configured data source'}
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  />
                  <ReferenceLine x={asOfDate} stroke="#f59e0b" strokeDasharray="3 3" label={{ position: 'top', value: 'Forecast as-of', fill: '#f59e0b', fontSize: 12 }} />
        ...
        <div className="space-y-8">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                ...
                <p className="text-xs text-slate-400 leading-relaxed mb-4">
                    2. <strong>Validation:</strong> Duplicate, stale, missing, invalid, and mixed-currency inputs fail closed.
                </p>
                <p className="text-xs text-slate-400 leading-relaxed">
                    3. <strong>Forecasting:</strong> The tested Python engine freezes assumptions and as-of dates before n8n persists each run.
                </p>
            </div>
        </div>
      </div>
